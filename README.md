...
- **朝一サマリ**: 全ラベルからスコア上位を抽出したうえで `global_summary_top_n` 件をGPTへ投入します（推奨 12 件）。同一ラベルの分散上限は **企業=1件 / テーマ=2件** がデフォルトで、Notion Targets の `MaxPick` で上書き可能です。
- **要約対象の前提**:
...
- ラベル別要約は `gpt-4o-mini` を利用
- `gpt-5.5` 相当の重いモデルは日次ブリーフでは基本不要
- 週次・月次の深掘り時のみ上位モデルを検討

### 冒頭ブリーフ投入件数の目安
- `limits.global_summary_top_n` で制御（推奨値 12）
- 8〜10: コスト重視
- 12: 推奨
- 15: 広めに拾う場合
- 20以上: 論点が散るため非推奨
