...
本リポジトリは Notion を単一情報源（single source of truth）として運用します。検索条件（Targets）とタグ/重要度ルール（Rules）は Notion DB で管理します。
...
設定/プロンプト読み込み
  ↓
Notion Targets/Rules を読み込み（唯一の参照元）
  ↓
...
### 任意（OpenAIモデル切り替え）
- `OPENAI_LABEL_SUMMARY_MODEL`（既定: `gpt-4o-mini`）
- `OPENAI_LABEL_SUMMARY_TEMPERATURE`（既定: `0.2`）
- `OPENAI_LABEL_SUMMARY_REASONING_EFFORT`（既定: `low`、GPT-5系時に有効）
- `OPENAI_LABEL_SUMMARY_VERBOSITY`（既定: `low`）
- `OPENAI_LABEL_SUMMARY_MAX_OUTPUT_TOKENS`（既定: `1200`）
- `OPENAI_MORNING_SUMMARY_MODEL`（既定: `gpt-5.4-mini`）
- `OPENAI_MORNING_SUMMARY_TEMPERATURE`（既定: `0.2`、非GPT-5時に有効）
- `OPENAI_MORNING_SUMMARY_REASONING_EFFORT`（既定: `medium`）
- `OPENAI_MORNING_SUMMARY_VERBOSITY`（既定: `medium`）
- `OPENAI_MORNING_SUMMARY_MAX_OUTPUT_TOKENS`（既定: `2200`）

#### 推奨設定
```bash
OPENAI_LABEL_SUMMARY_MODEL=gpt-4o-mini
OPENAI_MORNING_SUMMARY_MODEL=gpt-5.4-mini
OPENAI_MORNING_SUMMARY_REASONING_EFFORT=medium
OPENAI_MORNING_SUMMARY_VERBOSITY=medium
OPENAI_MORNING_SUMMARY_MAX_OUTPUT_TOKENS=2200
```

#### 高精度設定
```bash
OPENAI_MORNING_SUMMARY_MODEL=gpt-5.5
OPENAI_MORNING_SUMMARY_REASONING_EFFORT=medium
OPENAI_MORNING_SUMMARY_VERBOSITY=medium
```
...
```bash
OPENAI_MORNING_SUMMARY_MODEL=gpt-5.4-mini
OPENAI_MORNING_SUMMARY_REASONING_EFFORT=low
OPENAI_MORNING_SUMMARY_VERBOSITY=low
```
...
- 重要度は Notion Rule DB の `RuleType=importance` の Weight 合計で決定されます。
- `RuleType=importance` が0件の場合はフォールバックせず、`score=0 / importance=Low / importance_reasons=""` を設定します。
...
- `config/tagging.yml`: タグ辞書のローカル定義（Rules DBに未定義のタグを補助）
- `config/notion.yml`: Notionプロパティ名/タイプ、[AUTO]見出し名
- `config/settings.yml`: 実行上限・期間などの実行設定

## 起動時エラー（Notion未設定時）

以下が不足していると起動時に明確なエラーで停止します。
- `NOTION_TOKEN`
- `NOTION_TARGETS_DB_ID`
- `NOTION_RULES_DB_ID`
- `NOTION_ARTICLES_DB_ID`
